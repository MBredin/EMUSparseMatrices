...
//	printf("Floating rows: %ld\n", dims[0]%threads);
	...
	for(int i = 0; i < threads; i++){
		...
		ptrSize[i] = csrSplit(&nodes[i], dims, start, stop, values, rowInd, rowPtr, &loc_values[i], &loc_rowInd[i], &loc_rowPtr[i]);
		//printf("PtrSize: %ld\n", ptrSize[i]);
...
//	printf("Final Size: %ld", tempCnt);
	...
	timesOut[0] = 0;///splittime;
	timesOut[1] = 0;///soltime;
