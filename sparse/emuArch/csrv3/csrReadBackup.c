...
#if SOLUTIONTIME
long *csrSpMV(long *nodeId, long **emuRows);
#else
...
int main(){
	...
	//starttiming();
	#if READMATRIX
		//Arrays for row and column index and non-zero entries
		double **oMat = NULL;
		int* M = NULL; 
		int* N = NULL;
	        int* nz = NULL;
		oMat = readMat("pdb1HYS.mtx", nz, M, N);
	#else
		for(int i = 0; i < NODELETS; i++){
	...
	for(int i = 0; i < NODELETS; i++){
		#if SOLUTIONTIME
		solution[i] = cilk_spawn csrSpMV(&nodes[i], splitData[i]); //solution[i] = 
		#else
