...
void Solution(int id, long *data, long *rowindex, long *colindex, long *x, long *sol, long nz);
...
int main(int argc, char** argv){
	...
	for(int i = 0; i < CORES; i++){
		...
		//Process is child, execute the function
			...
			//printf("Child Spawned: %d PID: %d\n", i, getpid());
			for(int j = 0; j < Threads; j++)
				Solution(j * Threads + i, data, colIndex, rowIndex, x, solution, pointer[Matdim]);
			exit(0);
		}
	}
	for(int i = 0; i < CORES; i++)
...
void Solution(int id, long *data, long *colIndex, long *rowIndex, long *x, long *sol, long nz){
	//////////////////////////////////
	/////////////SOLUTION/////////////
	//////////////////////////////////
	for(int i = 0; i < nz; i++){
		sol[rowIndex[i]] += data[i] * x[colIndex[i]];
	}
	return;
}
