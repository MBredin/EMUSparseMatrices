...
void printTile(struct Tile t, int tileNum){
	...
	for(int i = 0; i < OMEGA; i++){
		...
		for(int j = 0; j < SIGMA; j++){
			printf("%2ld ",t.Data[i*OMEGA+j]);
		}
		printf("|   |");
		for(int j = 0; j < SIGMA; j++){
			printf("%2ld ",t.Ind[i*OMEGA+j]);
		}
		printf("|   |");
		for(int j = 0; j < SIGMA; j++){
			printf("%2ld ",t.BitF[i*OMEGA+j]);
		}
		printf("|\n");
	}
	printf("Y-Offset[");
...
void inPreSum(long *arr){
	long *arrCpy = malloc(OMEGA * sizeof(long));
	for(int i = 0; i < OMEGA; i++){
		arrCpy[i] = arr[i];
	}
	for(int i = 1; i < OMEGA; i++)
		arr[i] = arr[i-1] + arrCpy[i];
}
...
void fastSegSum(long *in, long *segOff){
	long *temp = malloc(OMEGA * sizeof(long));
	for(int i = 0; i < OMEGA; i++)
		temp[i] = in[i];
	inPreSum(in);
	for(int i = 0; i < OMEGA; i++)
		in[i] = in[i+segOff[i]] - in[i] + temp[i];
	free(temp);
}
...
void csr5_spmv(struct Tile currt, double *rowPtr, double *tilePtr, double *y, double *x, int tid);
...
int main(){
	...
	double *y = malloc(Orig.rowM * sizeof(double));
	for(int i = 0; i < Orig.rowM; i++)
		x[i] = i+1;

	for(int i = 0; i < tileCnt; i++){
		csr5_spmv(tileArr[i], rowPtr, tilePtr, y, x, i);
	}
	...
	*tilePLen = (int)tileCnt;
	...
	for(int i = 0; i < Orig.rowM; i++){
		...
		for(int j = 0; j < Orig.colN; j++){
			if(*(Orig.mat+i*Orig.colN+j) != 0){
				tileArr[cnt / tileSize].Data[cnt%OMEGA * OMEGA + (cnt/SIGMA)%OMEGA] = *(Orig.mat+i*Orig.colN+j);
				tileArr[cnt / tileSize].Ind[cnt%OMEGA * OMEGA + (cnt/SIGMA)%OMEGA] = j;
				if(locCnt == 0 || cnt%tileSize == 0)
					tileArr[cnt / tileSize].BitF[cnt%OMEGA * OMEGA + (cnt/SIGMA)%SIGMA] = 1;
				else
					tileArr[cnt / tileSize].BitF[cnt%OMEGA * OMEGA + (cnt/SIGMA)%SIGMA] = 0;
				cnt++;
				locCnt++;
			}
		}
	...
	for(int t = 0; t < tileCnt; t++){
		for(int i = 0; i < OMEGA; i++){
			...
			bitT[i] = 0;
			for(int j = 0; j < SIGMA; j++){
				tileArr[t].yOff[i] = tileArr[t].yOff[i] + tileArr[t].BitF[j * OMEGA + i];
				bitT[i] = bitT[i] || tileArr[t].BitF[j * OMEGA + i+1];
			}
		...
		if(tilePtr[t] < 0 || tilePtr[t] == -0.0){
			...
			for(int i = 0 ; i < OMEGA; i++)
				for(int j = 0; j < SIGMA; j++)
					length += tileArr[t].BitF[j * OMEGA + i];
		
			for(int i = 0 ; i < OMEGA; i++){
				for(int j = 0; j < SIGMA; j++){
					if(tileArr[t].BitF[j * OMEGA + i] == 1){
...
void csr5_spmv(struct Tile currt, double *rowPtr, double *tilePtr, double *y, double *x, int tid){
	...
	for(int i = 0; i < OMEGA; i++)
		temp[i] = 0;
	long *lastTemp = malloc(OMEGA * sizeof(long));
	//For color, 0 = green, 1 = blue, 2 = red
	long sum, ptr, color;
	for(int i = 0; i < OMEGA; i++){
		sum = 0;
		if(currt.BitF[i * OMEGA] == 0)
			color = 2;
		else if(currt.BitF[i * OMEGA + SIGMA - 1] == 0)
			color = 1;
		for(int j = 0; j < SIGMA; j++){
			ptr = j * OMEGA + i;
			sum += currt.Data[ptr] * x[currt.Ind[ptr]];
			if(currt.BitF[ptr+1] == 1 && color == 2){
				temp[i-1] = sum;
				sum = 0;
			}
			else if(currt.BitF[ptr+1] == 1){
				y[(int)tilePtr[tid] + (int)currt.yOff[i]] = sum;
				currt.yOff[i] = currt.yOff[i] + 1;
				sum = 0;
			}
		}
		lastTemp[i] = sum;
	}
	fastSegSum(temp, currt.yOff);
	for(int i = 0; i < OMEGA; i++){
		lastTemp[i] += temp[i];
		y[(int)tilePtr[tid] + (int)currt.yOff[i]] = lastTemp[i];
	}
	//free(temp);
	//free(lastTemp);
}
